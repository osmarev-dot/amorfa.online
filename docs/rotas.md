# Rotas principais — AMORFA

## Páginas públicas
- `/`
- `/antologia.html`
- `/discografia.html`
- `/sobre.html`
- `/ouvir.html`
- `/fragmentos.html`
- `/transmissoes.html`

## Álbuns da Antologia
- `/discografia/albuns/antologia/album-i.html` — Tudo Que Eu Uso Me Usa de Volta
- `/discografia/albuns/antologia/album-ii.html` — Pequenos Incêndios sob o Ar-Condicionado no Máximo
- `/discografia/albuns/antologia/album-iii.html` — Teologia dos Fracassos Afetivos em Cama de Solteiro
- `/discografia/albuns/antologia/album-iv.html` — Fantasmogênese
- `/discografia/albuns/antologia/album-v.html` — O Arranjo do Submundo
- `/discografia/albuns/antologia/album-vi.html` — Epílogo: Onde Se Guarda o Que Não Existe Mais

## EPs
- `/discografia/eps/dia-dos-namorados-macabro.html` — Dia dos Namorados Macabro

## Redirects legados
- `/album-i.html` ... `/album-vi.html`
- `/ep-dia-dos-namorados-macabro.html`