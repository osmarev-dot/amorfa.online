...
async function renderFragments(){
  ...
  if(!view)return;
  const items=list(await loadJSON('fragmentos.json',{items:[]})).filter(x=>(x.texto||x.conteudo||'').trim()||(x.imagem||x.image||'').trim()).slice(0,16);
  if(!items.length){
    document.querySelector('#fragment-stage')?.classList.add('is-empty');
    return;
  }
  document.querySelector('#fragment-stage')?.classList.remove('is-empty');

  let current=0,paused=false,timer=null;
  view.innerHTML=items.map((item,i)=>{
    const text=esc(item.texto||item.conteudo||'fragmento visual');
    const img=assetPath(item.imagem||item.image||'','fragmentos')||'assets/img/site/marionete_1.png';
    const tone=esc(item.tom||'fragmento');
    const date=esc(item.data||'');
    return `<article class="fragment-card ${i===0?'active':''}">
      <img src="${img}" alt="${text}">
      <div class="fragment-text"><span>${tone}</span><blockquote>${text}</blockquote><p>${date}</p></div>
    </article>`;
  }).join('');
...
async function renderTransmissions(){
  ...
  if(!root)return;
  const posts=list(await loadJSON('transmissoes.json',{posts:[]})).slice(0,6);
  if(!posts.length){
